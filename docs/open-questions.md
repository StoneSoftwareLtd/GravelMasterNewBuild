...
| **The basket pop-up's inside** | After Add to cart, the new pop-up shows the basket's own summary, which keeps the old site's styling (square green + and -, the "ADD" tiles). | Restyle `AddToCartComponent.cshtml` when the basket page is done, since other pages use it too |
