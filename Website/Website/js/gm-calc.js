/* ===== GravelMaster quantity calculator (Optima design) =====
   Views/Shared/_QuantityCalculator.cshtml, shared by the new homepage and product page. Moved unchanged from
   gm-home.js: the formulas are the live /calculator page's, and "Send me my estimate" posts to
   /email/sendcalculatorcalculation like its Email Results button.
   Plain JavaScript: the site's jQuery arrives later through RequireJS. */

/* The product type list shows the chosen type in its category's colour */
(function () {
  document.querySelectorAll('.gm-calc select[data-colour-select]').forEach(function (sel) {
    function apply() {
...
(function () {
  var root = document.querySelector('.gm-calc [data-calc]');
  if (!root) return;
